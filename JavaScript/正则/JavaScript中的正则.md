...
表达式主导在匹配当前文本的时候，每次只是检查正则表达式的一部分，举个例子：
...
在 NFA 主导的正则引擎中，我们想要在段落中匹配 ning，在 reg 中的意思实际上是 nine 或者 night 或者 ning。第一个元素是 n，它会反复遍历这个段落，直到找到 n 字符，然后是 i 字符。紧接着是 ne 分支，它会接着匹配，匹配到 e 字符的时候，就会失败，然后进入下一个分支，直到第三个分支全部匹配。表达式的在不同元素之间进行转化，这就是表达式主导
